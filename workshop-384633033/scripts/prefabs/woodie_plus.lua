...
local function onhealthdelta(inst, oldpercent, newpercent)
    ...
    local absorb = inst.components.health.absorb
    local redirect_percent = inst.components.beaverness.redirect_percent or 1
    local delta = (oldpercent-newpercent)*maxhealth/(1-absorb)*redirect_percent
    inst.components.beaverness:DoDelta(delta)
end
--/MOD
...
local function onbecamehuman(inst)
    ...
    inst.components.beaverness:StartTimeEffect(1, -.75 * inst.components.beaverness.max / TUNING.BEAVER_DRAIN_TIME) --MOD

    inst.components.locomotor.runspeed = TUNING.WILSON_RUN_SPEED
    inst.components.combat:SetDefaultDamage(TUNING.UNARMED_DAMAGE)
    inst.components.combat.bonusdamagefn = nil
    inst.components.health:SetAbsorptionAmount(0)
    inst.components.health.ondelta = nil --MOD
...
local function onbecamebeaver(inst)
    ...
    inst.components.health.ondelta = onhealthdelta --MOD
...
local function master_postinit(inst)
    ...
    inst.components.beaverness.redirect_percent = TUNING.BEAVER_REDIRECT_PERCENT --MOD
    inst.components.beaverness.IsStarving = function() return false end --MOD
